...
const Navbar = () => {
    ...
    const handleClickOutside = (event: MouseEvent) => {
        ...
        if (dropdownRef.current &&
            ...
            !(event.target as Element).closest('.dropdown-toggle') // Ignore the dropdown button
        ) {
            setProfileMenuOpen(false) // Close menu when clicking outside
        }
    }
    ...
    return (
        <>
            <div className='bg-tabbi-light-gray w-full border-b border-black z-20'>
                <WidthContainer>
                    <div className="relative bg-tabbi-light-gray flex w-full items-center justify-between pt-4 pb-2 gap-6">
                        ...
                        <div className='block lg:hidden h-full hover:cursor-pointer'>
                            <div
                        ...
                        <div className="hidden lg:flex space-x-3 items-center justify-center">
                            ...
                            <div
                                className={`absolute ${!profileMenuOpen ? 'bottom-1' : 'top-[3.9rem] md:top-[5rem]'} right-0 flex flex-col justify-center items-center gap-5 min-w-36 border-1 border-tabbi-dark-gray bg-tabbi-light-gray py-4 -z-[1] transition-all duration-[.2s]`}
                            >
                                <Link
                                    to="/user/profile"
            ...
            <div
                className={`${!navMenuOpen ? 'h-0 border-0 -translate-y-2' : 'h-32 py-4 border-y-1'} overflow-hidden flex flex-col justify-center items-center gap-4 pt-2 min-w-36 border-tabbi-dark-gray bg-tabbi-light-gray transition-all duration-[.2s]`}
            >
